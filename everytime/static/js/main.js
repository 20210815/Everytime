...
function toggleQuestion() {
  const questionOption = document.querySelector(".option .question");

  questionOption.classList.toggle("active"); // Toggle active class for styling
}
...
document.getElementById("writeArticleButton").addEventListener("click", function () {
  ...
  var formHtml = `
        <form class="write">
            ...
            <p>
                <textarea name="content" placeholder="에브리타임은 누구나 기분 좋게 참여할 수 있는 커뮤니티를 만들기 위해 커뮤니티 이용규칙을 제정하여 운영하고 있습니다. 위반 시 게시물이 삭제되고 서비스 이용이 일정 기간 제한될 수 있습니다. 
...
- 스포일러, 공포, 속임, 놀라게 하는 행위" class="smallplaceholder"></textarea>
            ...
            <ul class="option">
                ...
                <li title="질문" class="question" onclick="toggleQuestion()"></li>
            </ul>
            <div class="clearBothOnly"></div>
        </form>
...
function submitForm(formElement) {
  ...
  const title = formElement.querySelector("input[name='title']").value;
  const content = formElement.querySelector("textarea[name='content']").value;

  // Print the title and content to the console (for debugging)
  console.log("Title:", title);
  console.log("Content:", content);

  // Perform form submission (for example, using fetch API or traditional form submission)
  // Example using traditional form submission:
  formElement.submit();
}
